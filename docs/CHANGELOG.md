...
- `data modify storage macro:engine global.version set value "v2.2.0"` satırı eklendi — storage zaten varsa bile her load'da version üzerine yazılır
...
`V2.0.3` (büyük V) karşılaştırması yapılıyordu; engine `v2.0.3` (küçük v) saklıyor. Sonuç: kontrol daima başarısız, tüm injection koruması bypass ediliyordu. `v2.2.0` olarak düzeltildi.
